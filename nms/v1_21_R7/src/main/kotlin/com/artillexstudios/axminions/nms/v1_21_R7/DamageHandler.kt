...
object DamageHandler {
    ...
    fun damage(source: Minion, entity: Entity) {
        ...
        synchronized(DUMMY_ENTITY) {
            this.minion = source
            var f = 1 // ZMIANA: val -> var, bo zmieniamy wartość f
            ...
            val damageSource = nmsEntity.damageSources().mobAttack(DUMMY_ENTITY) // mobAttack zamiast noAggroMobAttack dla bezpieczeństwa
            ...
            if (f > 0.0f || f1 > 0.0f) {
                ...
                // --- NOWA LOGIKA DROPÓW (DO SKRZYNI MINIONA) ---
                val finalDamage = f.toFloat()

                if (nmsEntity is LivingEntity && (nmsEntity.health - finalDamage) <= 0) {
                    // 1. Pobieramy listę itemów (symulacja gracza) z obsługą Looting
                    val drops = LootHandler.dropPlayerKillLoot(entity, source.getTool())

                    // 2. Dodajemy do miniona
                    drops.forEach { item ->
                        source.addToContainerOrDrop(item)
                    }

                    // 3. Usuwamy standardowy drop (Bukkit API)
                    (entity as? org.bukkit.entity.Mob)?.lootTable = null
                }
                // -----------------------------------------------
                ...
                if (flag5) {
                    if (i > 0) {
                        if (nmsEntity is LivingEntity) {
                            ...
                            nmsEntity.push(
                                -Mth.sin((source.getLocation().yaw * 0.017453292f).toDouble()).toDouble() * i.toDouble() * 0.5,
                                0.1,
                                Mth.cos((source.getLocation().yaw * 0.017453292f).toDouble()).toDouble() * i.toDouble() * 0.5
                            )
                        }
                    }
                    ...
                    if (flag3) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if ((entityliving !is ArmorStand || !(entityliving).isMarker) && source.getLocation()
                                ...
                                if (entityliving.hurtServer((source.getLocation().world as CraftWorld).handle as ServerLevel, nmsEntity.damageSources().mobAttack(DUMMY_ENTITY), f4)) {
